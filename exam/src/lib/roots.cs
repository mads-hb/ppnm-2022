...
public static class RootFinding {
     ...
     */
    public static vector Newton(Func<vector,vector> f, vector x0, int max_iter=10000, double eps=1e-8){
        vector x = x0.copy();
        ...
        if(!conv){
            Console.Error.WriteLine("Eigenvalues for a matrix did not converge. Using unconverged result...");
        }
