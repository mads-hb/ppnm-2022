...
public static matrix random_symmetric(int n){
    matrix A = new matrix(n, n);
    Random random = new Random();
    int maximum = 10, minimum = -10;
    for(int i = 0; i < A.size1; i++){
        for(int j = 0; j < A.size2; j++) {
            // Matrix is symmetric. We only need to iterate one half.
            if (j >= i){
...
public static matrix random_diagonal(int n){
    matrix A = new matrix(n, n);
    Random random = new Random();
