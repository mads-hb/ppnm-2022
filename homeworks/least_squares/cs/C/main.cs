        ...
        using (var outfile = new System.IO.StreamWriter("fit.txt")){
            for (double value = 1; value < 16; value += 0.1){
                ...
                for (int k = 0; k < fs.Length; k++){
                    res_minus += (coefs[k]-Sqrt(covb[k, k])) * fs[k](value);
                    res += coefs[k] * fs[k](value);
                    res_plus += (coefs[k]+Sqrt(covb[k, k])) * fs[k](value);
                }
                outfile.WriteLine($"{value} {Exp(res_minus)} {Exp(res)} {Exp(res_plus)}");
            }
        }
        WriteLine($"The lambda parameter is estimated to be {-coefs[1]} +/- {Sqrt(covb[1,1])}.");
        WriteLine($"The half-life is then {Log(2)/-coefs[1]} +/- {-Log(2)/(coefs[1] + Sqrt(covb[1,1])) + Log(2)/coefs[1]} days");
        WriteLine("The modern value is 3.6319 days according to wikipedia.");
